      ...
      <div className='flex items-center justify-between'>
        <div className='flex items-center gap-3'>
          ...
          <div className='flex-1'>
            ...
            <p className='text-sm text-green-700 mb-1'>{selectedVoucher.description}</p>
            <div className='flex items-center gap-4 text-xs text-green-600'>
              <span className='font-medium'>{getDiscountDisplay()}</span>
              <span className='font-semibold'>Tiết kiệm: {formatPrice(discountAmount)}</span>
            </div>
          </div>
        </div>
        ...
        <button
          ...
          className='flex-shrink-0 p-1 text-green-600 hover:text-red-600 hover:bg-red-50 rounded-full transition-colors'
          title='Bỏ voucher'
        >
