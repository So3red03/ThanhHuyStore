...
const StripeCheckoutForm: React.FC<StripeCheckoutFormProps> = ({ clientSecret, handleSetPaymentSuccess }) => {
    ...
    stripe
      ...
      .then(async result => {
        if (!result.error) {
          ...
          // Stock is already updated atomically during order creation
          // No need for manual stock update here
  ...
  return (
    <form onSubmit={handleSubmit} id='payment-form'>
      ...
      <div className='py-4 text-center text-slate-700 text-xl font-bold'>Tổng cộng: {formatPrice(cartTotalAmount)}</div>
      <Button
        label='Thanh toán'
        isLoading={isLoading}
