          ...
          <div className='flex space-x-4'>
            {accessories.map(product => (
              ...
              >
                ...
                />
                <Image src={product.imgSrc} alt={product.name} width={130} height={170} className='h-auto mb-2' />
                <h4 className='text-sm font-medium text-center'>{truncateText(product.name)}</h4>
                <div className='text-sm mt-1'>
                  Giá niêm yết: <span className='font-bold text-[#d43232]'> {product.price.toLocaleString()}₫</span>
                </div>
                <button
                  onClick={() => handleSelectProduct(product.id)}
