          ...
          <ul className='list-none p-5 pb-8 pt-0'>
            {MenuItems.map(item => (
              <li key={item.title} className='mt-6'>
                ...
                {item.dashboardItem && item.dashboardItem.list && item.dashboardItem.list.length > 0 && (
                  <>
                    ...
                    >
                      ...
                      {openSubItems[item.dashboardItem.title] ? <MdExpandLess size={20} /> : <MdExpandMore size={20} />}
                    </button>
                    ...
                    >
                      ...
                      {item.dashboardItem.list.map(subItem => (
                        <AdminSideBarItem key={subItem.title} label={subItem.title} path={subItem.path} />
                      ))}
                    </ul>
                  </>
                )}
                    ...
                    >
                      ...
                      {item.subItem.list.map(subItem => (
                        <div key={subItem.title}>
                          <AdminSideBarItem label={subItem.title} path={subItem.path} icon={subItem.icon} />

                          {/* Kiểm tra nếu có danh sách con cho subItem */}
                          {subItem.list && subItem.list.length > 0 && (
                            <div>
                              <button
                                onClick={() => toggleChildItem(item.subItem.title, subItem.title)}
                                className='w-full flex justify-between gap-2 items-center p-4 hover:bg-slate-500 hover:text-white rounded-xl'
                              >
                                <span className='flex items-center gap-2'>
                              ...
                              <ul
                                className={`transition-all duration-400 ease-in-out overflow-hidden ml-4 ${
                                  openChildItems[`${item.subItem.title}-${subItem.title}`] ? 'max-h-screen' : 'max-h-0'
                                }`}
                              >
                                {subItem.list.map(child => (
                                  <AdminSideBarItem key={child.title} label={child.title} path={child.path} />
                                ))}
                              </ul>
                            </div>
                          )}
            ...
            <button
              ...
              className='p-4 w-full flex justify-stretch hover:bg-slate-500 hover:text-white items-center gap-2 my-1 rounded-xl'
            >
              <MdOutlineSettings size={20} /> <span>Cài đặt</span>
            </button>
            <button
