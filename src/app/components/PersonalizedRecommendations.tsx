...
const PersonalizedRecommendations: React.FC<PersonalizedRecommendationsProps> = ({ 
  ...
  return (
    <Container>
      <div
        className={`mt-12 relative rounded-lg overflow-hidden xl:mx-[50px] ${styles.recommendationsContainer}`}
        style={{
          background: 'linear-gradient(135deg, #667eea 0%, #764ba2 100%), url("data:image/svg+xml,%3Csvg width="60" height="60" viewBox="0 0 60 60" xmlns="http://www.w3.org/2000/svg"%3E%3Cg fill="none" fill-rule="evenodd"%3E%3Cg fill="%23ffffff" fill-opacity="0.1"%3E%3Ccircle cx="30" cy="30" r="4"/%3E%3C/g%3E%3C/g%3E%3C/svg%3E")',
          minHeight: '400px'
        }}
      >
        {/* Overlay để làm nền tối hơn */}
        <div className={`absolute inset-0 bg-black bg-opacity-20 ${styles.backgroundOverlay}`}></div>

        {/* Content */}
        <div className={`relative z-10 p-6 ${styles.content}`}>
          {/* Header với icon gradient */}
          <div className={`title mb-6 ${styles.title}`}>
            <h2 className={`flex items-center pb-3 text-2xl font-bold ${styles.titleText}`} style={{ color: 'rgb(4, 41, 122)' }}>
              <svg
                ...
                fill="none"
                className={`mr-3 ${styles.titleIcon}`}
              >
                <path
          ...
          <div className="product-list-swiper">
            ...
            >
              ...
              {/* Navigation buttons */}
              <div className="swiper-button-prev-recommendations swiper-button-prev !text-white !w-10 !h-10 !mt-[-20px] after:!text-lg after:!font-bold"></div>
              <div className="swiper-button-next-recommendations swiper-button-next !text-white !w-10 !h-10 !mt-[-20px] after:!text-lg after:!font-bold"></div>
            </Swiper>
          </div>
        </div>
      </div>
