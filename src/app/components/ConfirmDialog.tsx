...
const ConfirmDialog: React.FC<ConfirmDialogProps> = ({ isOpen, handleClose, children, onConfirm, alert = false }) => {
	return (
		<>
			{alert ? (
				<Dialog
				...
				>
					...
					<DialogActions className="flex justify-center items-center px-5">
						...
						<button
							onClick={onConfirm}
							className="px-5 py-3 mb-2 bg-slate-600 border-slate-600 text-white font-semibold text-sm transform-none rounded flex-1 hover:opacity-80"
						>
							Đăng nhập
						</button>
					</DialogActions>
				...
				>
					...
					<DialogActions className="flex justify-center items-center px-5">
						<button
							onClick={handleClose}
							className="bg-[#ececec] border-[#ececec] px-5 py-3 mb-2 font-semibold text-sm transform-none rounded flex-1"
						>
							Thoát
						</button>
						<button
							onClick={onConfirm}
							className="px-5 py-3 mb-2 bg-slate-600 border-slate-600 text-white font-semibold text-sm transform-none rounded flex-1"
						>
							Xác nhận
						</button>
					</DialogActions>
